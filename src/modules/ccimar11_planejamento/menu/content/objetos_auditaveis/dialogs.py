...
class MultipicadoresDialog(QDialog):
    """
    Diálogo para edição dos multiplicadores de materialidade, relevância e criticidade.
    """
    def __init__(self, parent=None, materialidade_peso=4, relevancia_peso=2, criticidade_peso=4):
        """
        Inicializa o diálogo de multiplicadores.
        ...
        # Campo para materialidade
        self.materialidade_edit = QLineEdit(str(self.materialidade_peso))
        self.materialidade_edit.setValidator(QIntValidator(1, 10))
        form_layout.addRow("Materialidade (peso):", self.materialidade_edit)
        
        # Campo para relevância
        self.relevancia_edit = QLineEdit(str(self.relevancia_peso))
        self.relevancia_edit.setValidator(QIntValidator(1, 10))
        form_layout.addRow("Relevância (peso):", self.relevancia_edit)
        
        # Campo para criticidade
        self.criticidade_edit = QLineEdit(str(self.criticidade_peso))
        self.criticidade_edit.setValidator(QIntValidator(1, 10))
        form_layout.addRow("Criticidade (peso):", self.criticidade_edit)
    ...
    def get_multiplicadores(self):
        """
        Obtém os valores dos multiplicadores definidos no diálogo.
        
        Returns:
            tuple: (materialidade_peso, relevancia_peso, criticidade_peso)
        """
        try:
            materialidade = int(self.materialidade_edit.text())
            relevancia = int(self.relevancia_edit.text())
            criticidade = int(self.criticidade_edit.text())
            
            return materialidade, relevancia, criticidade
        except ValueError:
            # Em caso de erro, retornar os valores padrão
            return 4, 2, 4
